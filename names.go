...
func searchMapForParts(list map[string]bool, nameParts *[]string, nameCommas *[]bool) (partsFound []string) {
	...
	if db1 {
		dbgo.Printf("partsFound %s, %(LF)\n", dbgo.SVar(partsFound))
	}
	for _, vv := range partsFound {
		if foundIndex := g_lib.LocationInArray(vv, *nameParts); foundIndex >= 0 {
			*nameParts = g_lib.RemoveAt(*nameParts, foundIndex)
			if (*nameCommas)[foundIndex] && foundIndex != len(*nameCommas)-1 {
				*nameCommas = g_lib.RemoveAt(*nameCommas, foundIndex+1)
			} else {
				*nameCommas = g_lib.RemoveAt(*nameCommas, foundIndex)
			}
		}
	}
	return
	...
	if len((*nameParts)) > 1 {
		for ii := len((*nameParts)) - 2; ii >= 0; ii-- {
			if np, ok := prefixListLookup[(*nameParts)[ii]]; ok && np {
				...
				(*nameParts) = g_lib.RemoveAt(*nameParts, ii+1)
				(*nameCommas) = g_lib.RemoveAt(*nameCommas, ii)
			}
		}
	}
}
